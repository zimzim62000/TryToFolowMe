...
bool Player::Update(float const dt, sf::RenderWindow* window)
{
	...
	float a, b;
	a = this->getPosition().x + this->getGlobalBounds().width / 2 - window->getSize().x / 2;
	b = this->getPosition().y + this->getGlobalBounds().height / 2 - window->getSize().y / 2;
	if (a < 0)a = 0;
	if (b < 0)b = 0;
	view.reset(FloatRect(a,b,window->getSize().x, window->getSize().y));
	window->setView(view);
	*/
